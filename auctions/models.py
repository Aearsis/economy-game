...
class Auction(models.Model):
    ...
    def effective_offer(self):
        ...
        """
        bids = self.bid_set.order_by("-amount", "placed")
        count = len(bids)
        if count == 0:
            return None, None
        elif count == 1:
    ...
    def highest_bid(self):
        ...
        """
        return self.bid_set.order_by("-amount", "placed")[0]
    ...
    def create(team: Team, data):
        ...
        with Transaction() as t:
            ...
            for item in data['auctioneditems']:
                ai = AuctionedItem(auction=auc, entity=item['entity'], amount=item['amount'] * int(item['coef']))
                if ai.amount > 0:
...
class AuctionedItem(models.Model):
    ...
    def block(self, t: Transaction, bidder: Team, coef=1):
        """
        Blocks the item for the highest bidder. It makes sense only when amount is negative.
        """
        if self.amount < 0:
